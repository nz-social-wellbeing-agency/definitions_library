...
Outputs:
- [IDI_Sandpit].[DL-MAA2016-15].[defn_hnz_waitlist]
...
/* Staging */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2016-15].[defn_hnz_waitlist]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2016-15].[defn_hnz_waitlist];
GO
...
via_application_uid AS (
	...
	FROM [IDI_Clean_20200120].[hnz_clean].[new_applications_household] h
	INNER JOIN [IDI_Clean_20200120].[hnz_clean].[new_applications] a
	ON h.[snz_application_uid] = a.[snz_application_uid]
	LEFT JOIN [IDI_Clean_20200120].[hnz_clean].[register_exit] e
...
SELECT [snz_uid]
	  ...
	  ,ID
INTO [IDI_Sandpit].[DL-MAA2016-15].[defn_hnz_waitlist]
FROM (
	SELECT *
...
/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2016-15].[defn_hnz_waitlist] (snz_uid);
GO

/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2016-15].[defn_hnz_waitlist] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
