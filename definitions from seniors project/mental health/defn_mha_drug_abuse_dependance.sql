/**************************************************************************************************
Title: Any indication of chronic or acute drug abuse or dependence
Author: Craig Wright
...
Outputs:
- [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_drug_abuse_or_dependence]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_solo]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_solo] (
	snz_uid	INT,
	event_date DATE,
)

/* Diagnosis or treatment used for bipolar and other conditions */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_multi]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_multi] (
	snz_uid	INT,
	event_date DATE,
)
...
INSERT INTO [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_solo] (snz_uid, event_date)
SELECT b.snz_uid
...
INSERT INTO [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_multi] (snz_uid, event_date)
SELECT a.[snz_uid]
		,[moh_pha_dispensed_date] AS event_date
FROM [IDI_Clean_202203].[moh_clean].[pharmaceutical] AS a
INNER JOIN [IDI_Metadata].[clean_read_CLASSIFICATIONS].[moh_dim_form_pack_subsidy_code] AS b
...
FROM [IDI_Clean_202203].[moh_clean].[pub_fund_hosp_discharges_event]
...
FROM [IDI_Clean_202203].[moj_clean].[charges] AS a
WHERE EXISTS (
	SELECT 1
	FROM [IDI_Sandpit].[DL-MAA2018-48].[ref_diagnosis] AS r
...
FROM [IDI_Clean_202203].[pol_clean].[nia_links] AS a
WHERE EXISTS (
	SELECT 1
	FROM [IDI_Sandpit].[DL-MAA2018-48].[ref_diagnosis] AS r
...
/* Add indexes */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_solo] (snz_uid);
GO
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_multi] (snz_uid);
GO

DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_drug_abuse_or_dependence]
GO

WITH multi_to_add AS (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_multi] AS m
	WHERE EXISTS (
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_solo] AS s
		WHERE m.snz_uid = s.snz_uid
	)
)
SELECT DISTINCT snz_uid, event_date
INTO [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_drug_abuse_or_dependence]
FROM (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA2018-48].[tmp_drug_solo]
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_drug_abuse_or_dependence] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_drug_abuse_or_dependence] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
