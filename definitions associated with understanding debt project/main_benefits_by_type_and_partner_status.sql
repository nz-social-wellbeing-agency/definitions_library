...
Outputs:
- [IDI_Sandpit].[DL-MAA2020-01].[tmp_main_benefit_final]
...
/* drop table before re-creating */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[tmp_main_benefit_final]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[tmp_main_benefit_final];
GO
...
SELECT k.*
	...
	,code.level3
	,code.level4
INTO [IDI_Sandpit].[DL-MAA2020-01].[tmp_main_benefit_final]
FROM (
...
/* recipient where role = single as no partner during period */
	...
	FROM [IDI_UserCode].[DL-MAA2020-01].[tmp_main_benefit_spell_input] ys -- yes, spell
...
/* receipt as role = partner */
	...
	FROM [IDI_UserCode].[DL-MAA2020-01].[tmp_main_benefit_partner_input] yp -- yes, partner
	LEFT JOIN [IDI_UserCode].[DL-MAA2020-01].[tmp_main_benefit_spell_input] ns -- no, spell
...
CREATE CLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA2020-01].[tmp_main_benefit_final] ([snz_uid])

ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].tmp_main_benefit_final REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
