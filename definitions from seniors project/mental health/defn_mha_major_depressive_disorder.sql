...
Outputs:
- [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_major_depressive_disorder]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_solo]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_solo] (
	snz_uid	INT,
	event_date DATE,
)

/* Diagnosis or treatment used for bipolar and other conditions */
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_multi]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_multi] (
	snz_uid	INT,
	event_date DATE,
)
...
INSERT INTO [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_multi] (snz_uid, event_date)
SELECT [snz_uid]
...
INSERT INTO [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_solo] (snz_uid, event_date)
SELECT a.[snz_uid]
...
/* Add indexes */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_solo] (snz_uid);
GO
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_multi] (snz_uid);
GO

DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_major_depressive_disorder]
GO

WITH multi_to_add AS (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_multi] AS m
	WHERE EXISTS (
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_solo] AS s
		WHERE m.snz_uid = s.snz_uid
	)
)
SELECT DISTINCT snz_uid, event_date
INTO [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_major_depressive_disorder]
FROM (
	SELECT snz_uid, event_date
	FROM [IDI_Sandpit].[DL-MAA2018-48].[tmp_depressive_solo]
...
/* Add index */
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_major_depressive_disorder] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2018-48].[defn_mha_major_depressive_disorder] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
