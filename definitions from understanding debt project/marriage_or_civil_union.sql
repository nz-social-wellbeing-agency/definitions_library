...
Outputs:
- [IDI_UserCode].[DL-MAA2020-01].[d2g_official_relationship]
...
/* Clear view */
IF OBJECT_ID('[DL-MAA2020-01].[d2g_official_relationship]','V') IS NOT NULL
DROP VIEW [DL-MAA2020-01].[d2g_official_relationship];
GO

CREATE VIEW [DL-MAA2020-01].[d2g_official_relationship] AS
WITH
marriage AS (
	...
	FROM [IDI_Clean_20200120].[dia_clean].[marriages] a
	INNER JOIN [IDI_Clean_20200120].[data].[personal_detail] b
	ON a.[partnr1_snz_uid] = b.snz_uid
	INNER JOIN [IDI_Clean_20200120].[data].[personal_detail] c
...
civil_union AS (
	SELECT [partnr1_snz_uid]
		  ...
		  ,DATEFROMPARTS(b.[snz_deceased_year_nbr], b.[snz_deceased_month_nbr], 28) AS [partnr1_deceased_date]
		  ,DATEFROMPARTS(c.[snz_deceased_year_nbr], c.[snz_deceased_month_nbr], 28) AS [partnr2_deceased_date]
	FROM [IDI_Clean_20200120].[dia_clean].[civil_unions] a
	INNER JOIN [IDI_Clean_20200120].[data].[personal_detail] b
	ON a.[partnr1_snz_uid] = b.snz_uid
	INNER JOIN [IDI_Clean_20200120].[data].[personal_detail] c
	ON a.[partnr2_snz_uid] = c.snz_uid
