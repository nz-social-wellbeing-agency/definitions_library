...
Outputs:
- [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education]
...
1) Industry training duration of enrollment can differ widely from expected duration of
   ...
   only enrollment.
2) Secondary and primary enrollment are not included. These can be found in the
   [IDI_Clean].[moe_clean].[student_enrol] table.
...
**************************************************************************************************/
/* Clear staging table */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging];
GO
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging]
FROM [IDI_Clean_20201020].[moe_clean].[enrolment]
...
FROM [IDI_Clean_20201020].[moe_clean].[targeted_training]
...
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging] (snz_uid)
GO

/* Condensed spells */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education];
GO
...
spell_starts AS (
	...
	FROM [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging] s1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging] s2
...
spell_ends AS (
	...
	FROM [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging] t1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1 
		FROM [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education_staging] t2
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education]
FROM spell_starts s
...
/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_enrolled_education] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
