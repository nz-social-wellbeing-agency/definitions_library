...
Outputs:
- [IDI_UserCode].[DL-MAA2021-49].[vacc_functional_disability]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2021-49].[tmp_functional_disability_list]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA2021-49].[tmp_functional_disability_list] (
	snz_uid INT,
...
INSERT INTO [IDI_Sandpit].[DL-MAA2021-49].[tmp_functional_disability_list]
...
WITH extracted_codes AS (
	...
	FROM [IDI_Adhoc].[clean_read_MOH_SOCRATES].[moh_support_needs_202110] AS s
	INNER JOIN [IDI_Clean_20211020].[security].[concordance] as c
	ON s.snz_moh_uid = c.snz_moh_uid
	LEFT JOIN [IDI_Adhoc].[clean_read_MOH_SOCRATES].[moh_needs_assessment_202110] AS n
...
WITH extracted_codes AS (
	...
	FROM [IDI_Clean_20211020].[moh_clean].[pub_fund_hosp_discharges_diag]
...
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2021-49].[tmp_functional_disability_list] (snz_uid);
GO

DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2021-49].[vacc_functional_disability]
GO
...
INTO [IDI_Sandpit].[DL-MAA2021-49].[vacc_functional_disability]
FROM date_ranked
WHERE ranking = 1
GO

CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2021-49].[vacc_functional_disability] (snz_uid);
GO
ALTER TABLE [IDI_Sandpit].[DL-MAA2021-49].[vacc_functional_disability] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
GO
