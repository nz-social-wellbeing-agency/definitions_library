...
Outputs:
- [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy]
...
/* Condensed spells */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging];
GO
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging]
...
/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging] (snz_uid);
GO

/* Condensed spells */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy];
GO
...
spell_starts AS (
	...
	FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging] s1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging] s2
...
spell_ends AS (
	...
	FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging] t1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1 
		FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy_staging] t2
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy]
FROM spell_starts s
...
/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_hnz_tenancy] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
