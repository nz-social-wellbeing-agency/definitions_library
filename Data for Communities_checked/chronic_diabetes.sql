...
AND (
	...
	OR SUBSTRING([moh_dia_clinical_code], 1, 3) IN ('O240', 'O241', 'O242', 'O243') /* pre-existing diabetes in pregnancy */
)
...
AND (
	...
	OR SUBSTRING([moh_pri_diag_clinic_code], 1, 3) IN ('O240', 'O241', 'O242', 'O243') /* pre-existing diabetes in pregnancy */
)
...
/* Clear before creation */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2016-15].[defn_tmp_nnpac_chronic_diags]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2016-15].[defn_tmp_nnpac_chronic_diags];
GO

/************************************ National Non-Admitted Patient Collection ************************************/
SELECT [snz_uid]
	  ,MIN([moh_nnp_service_date]) AS [moh_nnp_service_date]
INTO [IDI_Sandpit].[DL-MAA2016-15].[defn_tmp_nnpac_chronic_diags]
...
/* Clear before creation */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2016-15].[defn_chronic_diabetes]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2016-15].[defn_chronic_diabetes];
GO

/************************************ combined final table ************************************/

SELECT *
INTO [IDI_Sandpit].[DL-MAA2016-15].[defn_chronic_diabetes]
FROM (
/* public */
SELECT [snz_uid]
	  --,[event_id]
...
FROM [IDI_Sandpit].[DL-MAA2016-15].[defn_tmp_nnpac_chronic_diags]

) k

/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2016-15].[defn_chronic_diabetes] ([snz_uid]);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2016-15].[defn_chronic_diabetes] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
