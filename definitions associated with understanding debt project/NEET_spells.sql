...
- any_education_spells.sql --> [IDI_Sandpit].[DL-MAA2020-01].[d2g_enrolled_education]
- employment_spells.sql --> [IDI_Sandpit].[DL-MAA2020-01].[d2g_employed_spell]
Outputs:
- [IDI_Sandpit].[DL-MAA2020-01].[d2g_neet_spell]
...
/* Clear existing view */
IF OBJECT_ID('[DL-MAA2020-01].[d2g_NEET_spell_staging]','V') IS NOT NULL
DROP VIEW [DL-MAA2020-01].[d2g_NEET_spell_staging];
GO

CREATE VIEW [DL-MAA2020-01].[d2g_NEET_spell_staging] AS
SELECT [snz_uid]
      ,DATEFROMPARTS([snz_birth_year_nbr] + 18, [snz_birth_month_nbr], 15) AS [earliest_start_date]
	  ,CASE WHEN ([snz_deceased_year_nbr] IS NULL OR DATEDIFF(YEAR, [snz_birth_year_nbr], [snz_deceased_year_nbr]) > 65)
	   THEN DATEFROMPARTS([snz_birth_year_nbr] + 65, [snz_birth_month_nbr], 28)
	   ELSE DATEFROMPARTS([snz_deceased_year_nbr], [snz_deceased_month_nbr], 28) END AS [latest_end_date]
...
/* Non-NEET spell data */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2g_non_neet_spell_staging]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_non_neet_spell_staging];
GO

SELECT *
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2g_non_neet_spell_staging]
FROM (
	/* overseas */
	SELECT [snz_uid]
		...
		,'o' AS [source]
	FROM [IDI_Clean_20200120].[data].[person_overseas_spell] o
	...
	SELECT [snz_uid]
		...
		,[end_date]
		,'s' AS [source]
	FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_enrolled_education]
	...
	SELECT [snz_uid]
		...
		,[end_date]
		,'w' AS [source]
	FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_employed_spell]
) k
WHERE '2006-01-01' <= [end_date]
AND [start_date] <= '2026-12-31'
GO

CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2g_non_neet_spell_staging] (snz_uid);
GO


/* NEET spell data */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2g_neet_spell]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_neet_spell];
GO
...
s AS (
	...
	FROM [IDI_UserCode].[DL-MAA2020-01].[d2g_NEET_spell_staging] a
	WHERE NOT EXISTS(
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_non_neet_spell_staging] b
	...
	FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_non_neet_spell_staging] a
	WHERE NOT EXISTS(
		SELECT 1
		FROM [IDI_Sandpit].[DL-MAA2020-01].[d2g_non_neet_spell_staging] b
		WHERE a.snz_uid = b.snz_uid
	...
	AND NOT EXISTS(
		SELECT 1
		FROM [IDI_UserCode].[DL-MAA2020-01].[d2g_NEET_spell_staging] c
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2g_neet_spell]
FROM s
INNER JOIN e
...
/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2g_neet_spell] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_neet_spell] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
