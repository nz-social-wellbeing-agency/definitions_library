...
Output:
- [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_income_bnt_wff_tax_year]
...
2. runtime for [IDI_Sandpit].[DL-MAA2020-01].[tmp_benefit_tier2]:  18 mins
...
IF OBJECT_ID('[DL-MAA2020-01].[temp_income_tax_year]','V') IS NOT NULL
DROP VIEW [DL-MAA2020-01].[temp_income_tax_year];
GO

CREATE VIEW [DL-MAA2020-01].[temp_income_tax_year] AS
SELECT *
...
FROM [IDI_Clean_20201020].[data].[income_tax_yr_summary]
...
IF OBJECT_ID('tempdb..#tax_year') IS NOT NULL DROP TABLE #tax_year;
GO
...
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[tmp_benefit_tier2]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[tmp_benefit_tier2];
GO
...
FROM  [IDI_Clean_20201020].[msd_clean].[msd_second_tier_expenditure] t1
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[tmp_benefit_tier2]
FROM payment_breakdown
GROUP BY [snz_uid], [yr_tax]

CREATE INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[tmp_benefit_tier2] ([snz_uid]);
GO

 /*********************************************************************
 third tier benefits by tax year
 *********************************************************************/
IF OBJECT_ID('[DL-MAA2020-01].[tmp_benefit_tier3]','V') IS NOT NULL
DROP VIEW [DL-MAA2020-01].[tmp_benefit_tier3];
GO

CREATE VIEW [DL-MAA2020-01].[tmp_benefit_tier3] AS
SELECT [snz_uid]
	  ,DATEFROMPARTS([yr_tax],  1,  1) AS [start_date]
	  ,DATEFROMPARTS([yr_tax], 12, 31) AS [end_date]
 ...
 *********************************************************************/
IF OBJECT_ID('[DL-MAA2020-01].[tmp_wff]','V') IS NOT NULL
DROP VIEW [DL-MAA2020-01].[tmp_wff];
GO

CREATE VIEW [DL-MAA2020-01].[tmp_wff] AS
SELECT [snz_uid]  
      ,[tax_year]
...
FROM (
    ...
    FROM [IDI_Clean_20201020].[wff_clean].[fam_return_dtls]	
...
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[tmp_wff_neg_adj]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[tmp_wff_neg_adj];
GO

 SELECT *
       ,wff_pmt + neg_adj + COALESCE(lead_neg_deduct,0) AS wff_pmt_neg_adj
INTO [IDI_Sandpit].[DL-MAA2020-01].[tmp_wff_neg_adj]
FROM(
 SELECT *
 ...
 FROM [DL-MAA2020-01].[tmp_wff]
 ) a
GO

CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[tmp_wff_neg_adj] (snz_uid);
GO
...
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2gP2_income_bnt_wff_tax_year]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_income_bnt_wff_tax_year];
GO
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_income_bnt_wff_tax_year]
FROM [DL-MAA2020-01].[temp_income_tax_year] ir
FULL JOIN [IDI_Sandpit].[DL-MAA2020-01].[tmp_benefit_tier2] tier2
ON ir.[snz_uid] = tier2.[snz_uid]
AND ir.[start_date] = tier2.[start_date]
AND ir.[end_date] = tier2.[end_date]
FULL JOIN [DL-MAA2020-01].[tmp_benefit_tier3] tier3
ON ir.[snz_uid] = tier3.[snz_uid]
AND ir.[start_date] = tier3.[start_date]
AND ir.[end_date] = tier3.[end_date] 
FULL JOIN [IDI_Sandpit].[DL-MAA2020-01].[tmp_wff_neg_adj] wff
...
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_income_bnt_wff_tax_year] (snz_uid, start_date, end_date);
GO

/* Compress to save space  (takes 12 minutes) */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_income_bnt_wff_tax_year] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
...
DROP VIEW [DL-MAA2020-01].[tmp_wff];
GO
IF OBJECT_ID('[DL-MAA2020-01].[tmp_wff_neg_adj]','V') IS NOT NULL
DROP VIEW [DL-MAA2020-01].[tmp_wff_neg_adj];
GO
