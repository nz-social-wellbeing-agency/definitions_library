...
Outputs:
- [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_employed_spell]
...
/* Clear existing view */
IF OBJECT_ID('[DL-MAA2020-01].[d2gP2_employed_spell_staging]','V') IS NOT NULL
DROP VIEW [DL-MAA2020-01].[d2gP2_employed_spell_staging];
GO

/* Create staging */
CREATE VIEW [DL-MAA2020-01].[d2gP2_employed_spell_staging] AS
SELECT snz_uid
...
/* Condensed spells */
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2gP2_employed_spell]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_employed_spell];
GO
...
spell_starts AS (
	...
	FROM [IDI_UserCode].[DL-MAA2020-01].[d2gP2_employed_spell_staging] s1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1
		FROM [IDI_UserCode].[DL-MAA2020-01].[d2gP2_employed_spell_staging] s2
...
spell_ends AS (
	...
	FROM [IDI_UserCode].[DL-MAA2020-01].[d2gP2_employed_spell_staging] t1
	WHERE [start_date] <= [end_date]
	AND NOT EXISTS (
		SELECT 1 
		FROM [IDI_UserCode].[DL-MAA2020-01].[d2gP2_employed_spell_staging] t2
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_employed_spell]
FROM spell_starts s
...
/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_employed_spell] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2gP2_employed_spell] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
