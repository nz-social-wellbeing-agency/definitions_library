...
Outputs:
- [IDI_Sandpit].[DL-MAA2020-01].[d2g_sickness_benefit_spell]
...
IF OBJECT_ID('[IDI_Sandpit].[DL-MAA2020-01].[d2g_sickness_benefit_spell]','U') IS NOT NULL
DROP TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_sickness_benefit_spell];
GO
...
INTO [IDI_Sandpit].[DL-MAA2020-01].[d2g_sickness_benefit_spell]
FROM spell_starts s
...
/* Add index */
CREATE CLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2020-01].[d2g_sickness_benefit_spell] (snz_uid);
GO
/* Compress final table to save space */
ALTER TABLE [IDI_Sandpit].[DL-MAA2020-01].[d2g_sickness_benefit_spell] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE);
GO
