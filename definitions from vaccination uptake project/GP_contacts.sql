...
Outputs:
- [IDI_Sandpit].[DL-MAA2021-49].[vacc_GP_contacts]
- [IDI_UserCode].[DL-MAA2021-49].[vacc_GP_contacts_2020]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2021-49].[tmp_GP_contacts_list]
GO

CREATE TABLE [IDI_Sandpit].[DL-MAA2021-49].[tmp_GP_contacts_list] (
	snz_uid INT,
	visit_date DATE,
);
GO
...
INSERT INTO [IDI_Sandpit].[DL-MAA2021-49].[tmp_GP_contacts_list] (snz_uid, visit_date)
SELECT DISTINCT b.[snz_uid]
	,[acc_pay_first_service_date] AS visit_date
FROM [IDI_Clean_20211020].[acc_clean].[payments] AS a
INNER JOIN [IDI_Clean_20211020].[acc_clean].[claims] AS b
...
FROM [IDI_Clean_20211020].[moh_clean].[lab_claims] AS c
LEFT JOIN [IDI_Clean_20211020].[moh_clean].[pub_fund_hosp_discharges_event] AS b 
...
LEFT JOIN [IDI_Clean_20211020].[moh_clean].[nnpac] AS n 
...
FROM [IDI_Clean_20211020].[moh_clean].[pho_enrolment]
...
FROM [IDI_Clean_20211020].[moh_clean].[gms_claims]
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2021-49].[vacc_GP_contacts]
GO

SELECT DISTINCT snz_uid
	,visit_date
INTO [IDI_Sandpit].[DL-MAA2021-49].[vacc_GP_contacts]
FROM [IDI_Sandpit].[DL-MAA2021-49].[tmp_GP_contacts_list]
...
CREATE NONCLUSTERED INDEX my_index ON [IDI_Sandpit].[DL-MAA2021-49].[vacc_GP_contacts] (snz_uid)
GO
ALTER TABLE [IDI_Sandpit].[DL-MAA2021-49].[vacc_GP_contacts] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
GO
...
DROP VIEW IF EXISTS [DL-MAA2021-49].[vacc_GP_contacts_2020]
GO

CREATE VIEW [DL-MAA2021-49].[vacc_GP_contacts_2020] AS
--number of consults in 12 months - 1 Feb 2020 to 1 feb 2021
--0 to 10+ contacts in 12 months

SELECT snz_uid
	,IIF(COUNT(*) > 10 , 10, COUNT(*)) AS gpc
FROM [IDI_Sandpit].[DL-MAA2021-49].[vacc_GP_contacts]
WHERE visit_date >= '2020-02-01'
